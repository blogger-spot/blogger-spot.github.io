	...
	for (var i in tmp) {
		...
		tmp [i].forEach (function (e) {
			...
			if (odbc) {
				...
				if (odbc ["country:flag"] && odbc ["country:flag"].length) { var country_flag = []; for (var i in odbc ["country:flag"]) country_flag.push ("<img src='https://blogger-spot.github.io/asset/image/flag/" + odbc ["country:flag"][i] + ".jpg'/>"); html = html.split ('<div class="owl-carousel-thumbnail-flag"></div>').join ("<div class='owl-carousel-thumbnail-flag'>" + country_flag.join ('') + "</div>"); } else html = html.split ('<div class="owl-carousel-thumbnail-fla"></div>').join ("");
